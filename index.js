...
If type of the "searchElement" is object or array, you need to convert each element in the "array" 
to the string and then apply "includes" method with argument that will be also converted to the string.
...
const elementIsIncluded = (searchElement, array) => {
  let stringSearchElement = JSON.stringify(searchElement);
  if(typeof searchElement === "object" || typeof searchElement === "array"){    
    let stringArray = array.map(element => {
       return JSON.stringify(element)
    });
    return stringArray.includes(stringSearchElement);
  }else
